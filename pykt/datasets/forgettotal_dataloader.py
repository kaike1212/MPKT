...
class ForgetTotalDataset(Dataset):
    ...
    def __init__(self, file_path, input_type, folds, concept_num, max_concepts, code_type=None,
        ...
        super(ForgetTotalDataset, self).__init__()
        ...
        processed_data = file_path + folds_str + str(forget_window) + "_forgettotal.pkl"
    ...
    def __load_data__(self, sequence_path, folds, pad_val=-1):
        ...
        for i, row in df.iterrows():
            ...
            # TODO max_rgap
            max_rgap = max(max(max(lst) for lst in rgap),max_rgap)
            max_pcount = max(max(value for sublist1 in past_counts for sublist2 in sublist1 for value in sublist2),
                             max_pcount)
            max_acount = max(max(value for sublist1 in apast_counts for sublist2 in sublist1 for value in sublist2),
                             max_acount)
    ...
    def calC(self, row):
        ...
        for row_skill, t, r in zip(row_skills, timestamps, responses):
            t = int(t)
            r = int(r)
            rp, sp, pc = [], [], []
            for s in row_skill:
            ...
            for s in row_skills:
                ...
                for fw in forget_window_milliseconds:
                    ...
                    if j == 0:
                        ...
                        pc[i][j][0] = self.log2(count - pc[i][j - 1][0])
                        pc[i][j][1] = self.log2(acount - pc[i][j - 1][1])
                    j = j + 1
                pc[i][j][0], pc[i][j][1] = self.log2(len(dskill[s]) - pc[i][j - 1][0]), self.log2(
                    len(adskill[s]) - pc[i][j - 1][1])
                # 添加s
                if s != -1:
                    dskill[s].append(t)
                    if r == 1:
